...
ktrace -i -f ping-no-policy.ktrace -t u ${BUILD_DIR}/c18n_ping_client

echo "Test ping with policy"
export LD_C18N_PRELOAD=${BUILD_DIR}/libpolicy.so
ktrace -i -f ping-with-policy.ktrace -t u ${BUILD_DIR}/c18n_ping_client

echo "Teardown"
kill $SERVER_PID

kdump -H -f ping-no-policy.ktrace > ping-no-policy.txt
kdump -H -f ping-with-policy.ktrace > ping-with-policy.txt
